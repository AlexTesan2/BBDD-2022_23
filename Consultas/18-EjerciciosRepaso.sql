...
-- Hace una llamada a un procedimiento porque se le llama mediante la sentencia CALL y no espera recibir nada.
...
-- si, las variables no son lo mismo que los errores

# 7. ¿Para qué podemos utilizar un cursor en MySQL?


/*En MySQL, un cursor es un objeto que permite recorrer los resultados de una consulta SELECT de forma secuencial,
 permitiendo así el procesamiento de cada fila individualmente. Los cursores pueden ser útiles en situaciones en 
 las que se necesita realizar operaciones más complejas en los datos obtenidos a través de una consulta.*/
...
-- 0, el procedimiento test llama al procedimiento incrementor, pero incrementor no espera recibir ningun parametro
...
# Escriba un procedimiento llamado estadísticas_productos_vendidos que para cada uno de los productos de la tabla producto 
#calcule la cantidad total de unidades que se han vendido y almacene esta información en la tabla productos_vendidos.
#
...
# Calcular la cantidad total de productos vendidos. En este paso será necesario utilizar la función cantidad_total_de_productos_vendidos 
#desarrollada en el ejercicio 2.
# Insertar en la tabla productos_vendidos los valores del código de producto y la cantidad total de unidades que se han vendido 
